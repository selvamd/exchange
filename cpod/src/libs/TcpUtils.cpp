...
int32_t InitializeAcceptSocketTCP(const char * interface, in_port_t port, int & accept_socket)
{
    ...
    fcntl(accept_socket, F_SETFD, FD_CLOEXEC);
...
int32_t acceptConnectionTCP(int accept_socket, int & client_socket, sockaddr_in & client_address)
{
    ...
    client_socket = accept(accept_socket, 
...
int32_t connectTCP(const char * host, in_port_t port, int & sock)
{
    ...
    if (sock < 0)
    {
        LOG_ERROR << "Error creating socket for host:port => " << host << ":" << port << LOG_END;
        return SOCKET_CREATE_TCP_UTILS_ERROR;
    }
    ...
    if (rc <= 0)
    {
        LOG_ERROR << "Invalid address" << LOG_END;
        return INVALID_ADDRESS_TCP_UTILS_ERROR;
    }
    address.sin_port = htons(port);
    ...
    if (connect(sock, (struct sockaddr *) & address, sizeof(address)) < 0)
    {
        if (errno != ECONNREFUSED &&
            errno != EINPROGRESS )
        {
            LOG_DEBUG << "Error in connect to port:" << port << " errno: " << errno << LOG_END;
        }
...
/**
 * \brief Initiate non-blocking TCP connection 
 *
 * \param host - host ip address
 * \param port - host port
 * \param sock - reference to store socket file descriptor, set < 0 on error
 ...
 * \return < 0 on eror
 */
int32_t connectTCPNonBlocking(const char * host, in_port_t port, int & sock)
{
    sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (sock < 0)
    {
    ...
    if (makeSocketNonBlocking(sock) < 0)
    {
        LOG_ERROR << "Error setting socket nonblocking: " << sock << LOG_END;
        return SOCKET_CREATE_TCP_UTILS_ERROR;
    }
...
int32_t connectTCPByName(const char * hostname, in_port_t port, int & sock)
{
    struct hostent* host_info = gethostbyname(hostname);

    if (!host_info)
    {
        LOG_ERROR << "Invalid hostname" << LOG_END;
        return INVALID_ADDRESS_TCP_UTILS_ERROR;
    }

    unsigned char* host_address = (unsigned char*) host_info->h_addr_list[0];

    if (host_address)
    {
        std::stringstream ip_address;
        ip_address << (int)host_address[0] << "."
                   << (int)host_address[1] << "."
                   << (int)host_address[2] << "."
                   << (int)host_address[3] << std::ends;

        return connectTCP(ip_address.str().c_str(), port, sock);
    }
    else
    {
        LOG_ERROR << "Hostname has no address" << LOG_END;
        return INVALID_ADDRESS_TCP_UTILS_ERROR;
    }
}
...
{
    //LOG_DEBUG << "Setting socket " << socket << " to nonblocking" << LOG_END;
...
{
    //LOG_DEBUG << "Setting socket " << socket << " to blocking" << LOG_END;
